...
Fokusområden är det ledaren väljer när frågan är "vad ska vi träna i dag?" (berättelse 01). Samma lista används för att tagga övningar i fältet `fokusomraden` (`content/ovningar/README.md`). Generatorn väljer övningar som har minst ett av de valda fokusområdena (berättelse 02, kriterium 10).
...
- **Ett fokusområde beskriver innehåll, inte passdel.** Uppvärmning, huvuddel och avslutning är delar av passet. De definieras i `passuppbyggnad.md` i nästa steg och ska inte användas som fokusområden. Undantaget är `lek`, som både är ett innehåll och ofta förekommer i uppvärmningen.
...
| `nickspel` | Nickspel | Nicka bollen i anfall och försvar: teknik, tajming och att våga. Se åldersbegränsningen nedan. |
...
| `skadeforebyggande` | Skadeförebyggande | Övningar för knä, fotled och bål som minskar skaderisken, oftast som en del av uppvärmningen. Ett känt exempel i Sverige är programmet Knäkontroll. Jag har inte hämtat källan till programmet i det här uppdraget. |
...
| `avslut` | K | K | K | K | K |
| `nickspel` | – | – | R (begränsat, se nedan) | R | R |
...
| `snabbhet` | R | R | R | R | R |
| `uthallighet` | – | – | – | R | R |
| `skadeforebyggande` | – | – | R | K | K |
| `lek` | K | K | R | R | R |

### Varför tabellen ser ut så här

Allt nedan är min bedömning, grundad i `aldrar-och-fokus.md` och i spelformernas regler i `spelformer.md`.
...
- **8–9 år:** målvakten och retreatlinjen införs i 5 mot 5. Därför blir `malvaktsspel` och `speluppbyggnad` relevanta, i enkel form. Fasta situationer är fortfarande bara driv eller pass längs marken och behöver inte egen träning.
- **10–12 år:** 7 mot 7 har straffområde, frisparkar och retreatlinje, och målvakten får inte sparka ut bollen ur händerna. Spelförståelsen i grupp blir kärnan. `skadeforebyggande` införs som förberedelse, främst landningsteknik och kroppskontroll.
...
- **`nickspel`** följer mitt försiktiga förslag i `aldrar-och-fokus.md`: inte för 6–9 år och begränsat för 10–12 år. Frågan är öppen och ska avgöras av användaren. Om beslutet blir ett annat ändras bara raden för `nickspel`.

## Hur övningar taggas

Det här är mina förslag. Reglerna med ID skrivs i `generatorregler.md` i nästa steg.

1. En övning har **ett till tre** fokusområden. Det första i listan är övningens huvudfokus.
2. En övning får bara taggas med fokusområden som är **K eller R för hela övningens åldersspann**. En övning för 8–12 år får alltså inte taggas med `fasta-situationer`, eftersom det är "–" för 8–9 år.
3. Taggen ska beskriva vad spelarna **faktiskt övar mest** i övningen, inte allt som kan hända i den. Ett smålagsspel 4 mot 4 med kantzoner taggas `spelbarhet`, inte `avslut`, även om det görs mål.
...
Förslag till produktägaren och UX-designern:
...
Det här skiljer sig från berättelse 01, kriterium 9, där alla definierade fokusområden visas. Se rapporten.
